...
in
{
  ...
  options.desktop.pkgs.emulators = {
    wine = {
  ...
  config = lib.mkIf config.desktop.pkgs.emulators.enable {
    environment.systemPackages = with pkgs;
      lib.optionals config.desktop.pkgs.emulators.wine.enable [
        wine
        winetricks
      ] ++ lib.optionals config.desktop.pkgs.emulators.gaming.pcsx2.enable [
        pcsx2wrap
      ] ++ lib.optionals config.desktop.pkgs.emulators.gaming.lutris.enable [
        gamescope
        lutris
        mangohud
        mangojuice
