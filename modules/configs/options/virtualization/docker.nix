...
lib.mkIf config.preconfs.pkgs.virtualisation.docker.enable {
  ...
  users.extraGroups.docker.members = config.preconfs.pkgs.virtualisation.docker.users;
  users.users = builtins.listToAttrs (map (userName: {
    name = userName;
    value.extraGroups = [ "docker" ];
  }) config.preconfs.pkgs.virtualisation.docker.users);
}